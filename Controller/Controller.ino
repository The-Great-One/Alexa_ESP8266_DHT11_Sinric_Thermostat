...
void turnOn(String deviceId) {
  ...
  if (deviceId == "5e904bedce60582b5f8f35c3") // Device ID of first device
  {  
    ...
    case WStype_TEXT: {
        ...
        if(action == "setPowerState") { // On or Off
          ...
          else if(value>="31")             // Check if value is greater than or equal to 31°C. If so then set value to 30°C.
          {
            thirty(deviceId);
            Serial.println("Set to 30");
            setSetTemperatureSettingOnServer("5e904bedce60582b5f8f35c3", 30, "CELSIUS", roomtemp, humid);
          }
          else if(value<="19")             // Check if value is lower than or equal to 19°C. If so then set value to 20°C.
          {
            twenty(deviceId);
            Serial.println("Set to 20");
            setSetTemperatureSettingOnServer("5e904bedce60582b5f8f35c3", 20, "CELSIUS", roomtemp, humid);
          }
        }
...
// setPoint: Indicates the target temperature to set on the termostat.
  setSetTemperatureSettingOnServer("5e904bedce60582b5f8f35c3", currenttemp, "CELSIUS", temperature, humidity);  
